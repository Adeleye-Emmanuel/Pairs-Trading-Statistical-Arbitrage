...
class SignalGenerator:
    ...
    def __init__(self, lookback_days=60, entry_threshold=0.95, exit_threshold=0.5, 
                 max_holding_days=63, use_fitted_copula=True):
        
        self.lookback_days = lookback_days
        self.entry_threshold = entry_threshold
        self.exit_threshold = exit_threshold
        self.max_holding_days = max_holding_days
        self.use_fitted_copula = use_fitted_copula
        if not self.use_fitted_copula:
            print("WARNING: Using student-t copula approximation for all pairs (ignoring fitted copula types)")
    ...
    def calculate_conditional_probabilities(self, u, v, copula_model):
        ...
        if not self.use_fitted_copula:
            ...
            try:
                if hasattr(params_obj, "df") and hasattr(params_obj, "rho"):
                    df = float(params_obj.df)
                    rho = float(params_obj.rho[0]) if isinstance(params_obj.rho, np.ndarray) else float(params_obj.rho)
                elif hasattr(params_obj, "rho"):
                    rho = float(params_obj.rho[0]) if isinstance(params_obj.rho, np.ndarray) else float(params_obj.rho)
                    df = 30 # simulating high df for gaussian
                else:
                    return np.nan # As we cannot accurately estimate for archimedian copulas
    ...
    def generate_signal(self, prices_df, pair_data, 
        ...
        mode = "Student t-approximation" if not self.use_fitted_copula else copula_type
        print(f"Generating signals for {pair_name} (using {mode})...")
...
if __name__ == "__main__":
    ...
    test_prices_df, test_returns_df = selector.get_test_data()

    # Option 1: Use fitted copula types (Frank, Gumbel, Student-t, etc.)
    print("\n" + "="*60)
    print("SIGNAL GENERATION - Using Fitted Copula Types")
    print("="*60)
    signal_gen_fitted = SignalGenerator(use_fitted_copula=True)
    signals_fitted = signal_gen_fitted.generate_batch_signals(test_prices_df, selected_pairs)
    
    # Option 2: Force Student-t for all pairs (approximation)
    print("\n" + "="*60)
    print("SIGNAL GENERATION - Using Student-t Approximation")
    print("="*60)
    signal_gen_studentt = SignalGenerator(use_fitted_copula=False)
    signals_studentt = signal_gen_studentt.generate_batch_signals(test_prices_df, selected_pairs)
    
    # for pair_name, signals in signals_fitted.items():
    ...
    for pair_name in signals_fitted.keys():
        sig_fitted = signals_fitted[pair_name]
        sig_studentt = signals_studentt[pair_name]
        
        if sig_fitted is not None and sig_studentt is not None:
            entries_fitted = sig_fitted['entry_signal'].sum()
            entries_studentt = sig_studentt['entry_signal'].sum()
            
            print(f"\n{pair_name}:")
            print(f"  Fitted copula: {entries_fitted} signals")
            print(f"  Student-t approx: {entries_studentt} signals")
            print(f"  Difference: {entries_fitted - entries_studentt} signals")